...
def generateHeaderFile(execList, fileName):
	...
	for progName in execList:
		...
		# Increment the program index
		progCount += 1
		
		# Call getHexDump() to get the hexcode for each program
		print "Retrieving hexdump of program " + progName
		
		progHexcode = getHexDump(progName)
		
		print "Hexdump retrieved"
		
		# Get the length of the program
		lengthOfProg = len(progHexcode.split(','))
		
		# Add the length to progLens list
		progLens.append(str(lengthOfProg))
		
		# Write to the array
		headerFile.write("new unsigned char[" + str(lengthOfProg) + "] {" + progHexcode + "},")
		headerFile.write("\n")
	...
	for progName in execList:
		...
		# Add to the array the size of each program
...
def compileFile(binderCppFileName, execName):
	...
	# Check if compilation was successful or failed
	if exit_code == 0:
...
if len(sys.argv) < 2:
	...
	print "USAGE: " + sys.argv[0] + " <PROG1> <PROG2>...<PROGN>"
	exit()
